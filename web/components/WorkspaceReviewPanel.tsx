    ...
    <aside aria-label="Preserved agent workspaces" style={{ position: "absolute", inset: "16px 16px 16px auto", zIndex: 30, width: 420, maxWidth: "calc(100% - 32px)", overflow: "auto", padding: 16, borderRadius: 12, border: "1px solid #374151", background: "#0F172A", boxShadow: "0 18px 40px rgba(0,0,0,.45)" }}>
      ...
      {selected && changes && (
        <section style={{ marginTop: 12, padding: 10, border: "1px solid #4B5563", borderRadius: 8 }}>
          ...
          {changes.untrackedFiles.map((file) => { const preview = changes.untrackedPreviews.find((item) => item.path === file); return <div key={file}><label style={{ display: "block", margin: "6px 0", fontSize: 12 }}><input type="checkbox" checked={selectedUntracked.includes(file)} onChange={(event) => setSelectedUntracked((current) => event.target.checked ? [...current, file] : current.filter((value) => value !== file))} /> {file} <small style={{ color: "#FDE68A" }}>new · {preview?.kind ?? "unavailable"} · {preview?.size ?? 0} bytes</small></label>{preview?.preview&&<details><summary style={{fontSize:11}}>Preview</summary><pre style={{fontSize:10,whiteSpace:"pre-wrap",maxHeight:160,overflow:"auto"}}>{preview.preview}</pre></details>}</div>; })}
