...
import { createMemory, deleteMemoryEntry, listInbox, listMemory, markMailboxMessageRead, sendMailboxMessage, updateMemoryEntry } from "@/lib/auth";
...
export default function AgentContextPanel({ onClose }: { onClose: () => void }) {
  ...
  const [retentionDays, setRetentionDays] = useState<number | undefined>(30);
  ...
  async function save() { if (!content.trim()) return; setBusy(true); setError(null); try { if (tab === "memory") await createMemory(scope, content, agentId, retentionDays); else await sendMailboxMessage({ senderAgentId: agentId, recipientAgentId: recipient, kind, body: content, replyToId }); setContent(""); setReplyToId(undefined); await refresh(); } catch (e) { setError(e instanceof Error ? e.message : "Operation failed"); } finally { setBusy(false); } }
  return <aside aria-label="Agent memory and mailbox" style={{ position:"absolute", inset:"16px 16px 16px auto", zIndex:31, width:420, maxWidth:"calc(100% - 32px)", overflow:"auto", padding:16, borderRadius:12, border:"1px solid #374151", background:"#0F172A", boxShadow:"0 18px 40px rgba(0,0,0,.45)" }}>
    ...
    <div style={{display:"grid",gap:8,marginTop:14}}>{tab==="memory"?memories.map(item=><article key={item.id} style={{padding:9,background:"#111827",border:"1px solid #374151",borderRadius:7,fontSize:12}}>{item.content}<div><button onClick={async()=>{const next=window.prompt("Edit memory",item.content);if(next!==null){await updateMemoryEntry(item.id,next);await refresh();}}}>Edit</button> <button onClick={async()=>{if(window.confirm("Delete this memory?")){await deleteMemoryEntry(item.id);await refresh();}}}>Delete</button></div></article>):messages.map(item=><article key={item.id} style={{padding:9,background:"#111827",border:"1px solid #374151",borderRadius:7,fontSize:12}}><strong>{item.senderAgentId} → {item.recipientAgentId} · {item.kind}</strong><p>{item.body}</p><button onClick={()=>{setReplyToId(item.id);setRecipient(item.senderAgentId);}}>Reply</button> {item.status!=="read"&&<button onClick={async()=>{await markMailboxMessageRead(item.id);await refresh();}}>Mark read</button>}</article>)}</div>
  </aside>;
}
