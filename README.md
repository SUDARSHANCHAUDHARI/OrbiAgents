...
In **Tasks → Supervisor planner**, a local model can propose 1–6 sequential steps from a brief. Review all steps and the assigned running project agent, then approve dispatch. Work stays on one agent/workspace so later steps can use earlier changes. Workers receive a one-task, expiring loopback reporting capability; their completion reports advance the plan, while blocked work and ambiguous deliveries pause for review. Retries require an operator action and are rejected before dispatch once two attempts are consumed. Cancelling stops future dispatch and revokes reporting capabilities, but does not stop an already running agent. Plans are session-only; assigned tasks/results remain in Hive after restart. Reports are worker assertions, not independent test certification, and existing provider permissions remain necessary.

In **Settings → Voice control**, transcribed text is editable and can be explicitly sent to a selected running project agent after confirmation. The app rechecks consent, project membership and text bounds before delivery. Recording alone never dispatches work. Unlike local transcription, agent dispatch may forward text to a cloud provider and retain it independently; the confirmation explains this boundary.
...
The M6 source audit now enforces the Electron sandbox, context isolation, disabled Node/webview access, navigation/new-window denial, trusted-sender IPC, and restrictive CSP. Permissions fail closed except for audio-only microphone access from the trusted renderer while explicit consent and all local transcription prerequisites are active. Every audited compact form control has an accessible name; first-run onboarding is modal to assistive technology; focus, reduced motion, terminal screen-reader mode, Monaco labels, and a DOM alternative to the office canvas are present. Production budgets cap the initial renderer entry at 700 KB raw/130 KB gzip and CSS at 35 KB raw/10 KB gzip, with PixiJS, xterm, Monaco, settings, and operations panels delivered as separate lazy chunks. This is a code/build audit, not a substitute for the manual screen-reader, keyboard, signed-package, clean-account, real-CLI, native-speaker, Slack-workspace, and microphone acceptance listed in the macOS runbook.
