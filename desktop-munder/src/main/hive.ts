...
export class HiveManager {
  ...
  ensureHive(): void {
    ...
    // Raw crash tails may contain provider output, tokens, paths or prompts.
    // Keep them local instead of committing them into the hive repository.
  ...
  private injectedPrompt(
    ...
    const runtimeLine = rt
      ? `RUNNING BUILD: OrbiAgents v${rt.version}, ${rt.packaged ? 'packaged app' : 'local dev build'}${rt.appPath ? `, from ${rt.appPath}` : ''}. Say this version if asked which one is running, and do not assume behaviour from an older one. A local dev build inherits the launching shell's environment (umask included) where a packaged app does not, so file modes and inherited env can legitimately differ between the two. \`log.jsonl\` records an \`app-start\` event on every launch, which is how you spot a restart or a build switch.`
      : '';
  ...
  /** Record only abnormal agent exits, keeping raw output outside hive history. */
  recordAgentExit(
    agentId: string,
    ...
    const abnormal = (typeof signal === 'number' && signal !== 0)
    ...
    if (tail && tail.length) {
      try {
        ...
        const path = join(dir, `${stamp}-${safeId}.log`);
        const header = [
          `agent:    ${agentId}`,
        ...
        writeFileSync(path, header + tail, 'utf8');
        tailPath = path;
      } catch { /* diagnostics must never break teardown */ }
    }

    this.appendLog({
      kind: 'agent-exit', agentId, exitCode: exitCode ?? null,
      signal: signal ?? null, abnormal: true, tailPath
    });
  }
