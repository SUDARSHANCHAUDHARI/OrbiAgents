...
/** Locate the directory that actually holds a skill before walking a GitHub
 * repository-root source, so one skill's safety caps do not apply to the whole
 * repository. Directory listing is injected to keep the policy testable. */
export async function resolveRepoRootSkillDir(
  ...
  for (const candidate of candidates) {
    try {
      ...
      // A missing candidate is not fatal; continue through the known layouts.
    }
  }
  return {
    unsupported: true,
