 ...
 *  physical file is shared by every session query. */
...
/** Parse each complete line once into the unfiltered and owning-session totals. */
...
function readFileUsage(dir: string, file: string): FileUsageEntry | null {
  ...
  const entry: FileUsageEntry = fromScratch
    ...
    : {
        ...
        all: { ...cached!.all },
        perSession: new Map(Array.from(cached!.perSession, ([key, value]) => [key, { ...value }]))
      };
  try {
