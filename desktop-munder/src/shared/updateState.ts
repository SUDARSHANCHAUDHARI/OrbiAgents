...
export const REPO = 'SUDARSHANCHAUDHARI/OrbiAgents';
...
export function installerUrl(version: string, platform: string, arch: string): string {
  ...
  const file = platform === 'darwin' ? `OrbiAgents-${v}-mac-${arch}.dmg`
    : platform === 'win32' ? `OrbiAgents-${v}-win-x64-setup.exe`
    : `OrbiAgents-${v}-linux-x86_64.AppImage`;
  ...
  switch (status?.state) {
    ...
    case 'downloaded':
      return {
        ...
        detail: `Restart OrbiAgents to finish updating from v${v}.`,
...
export function manualInstallSteps(platform: string): { os: string; steps: string[] } {
  if (platform === 'darwin') {
    return {
      os: 'macOS',
      steps: [
        'Open the .dmg and drag OrbiAgents onto Applications. Choose Replace when asked.',
  ...
  if (platform === 'win32') {
    return {
      ...
      steps: [
        ...
        'Open OrbiAgents again and pick the same project.'
      ]
    };
  }
  return {
