...
export function workerFrame(direction, step, accent) {
  ...
  const variant = WORKER_COLORS.indexOf(accent) % 5;
  ...
  if (variant === 0) {
    ...
    rect(5, 1, 2, 3, shade); rect(11, 1, 2, 3, shade);
    rect(5, 1, 2, 1, accent); rect(11, 1, 2, 1, accent);
  } else if (variant === 2) {
    rect(2, 5, 1, 4, accent); rect(15, 5, 1, 4, accent);
  } else if (variant === 3) {
    rect(6, 1, 6, 2, shade); rect(7, 0, 4, 1, accent);
  } else {
    rect(7, 0, 4, 1, accent); rect(6, 1, 6, 2, shade);
    rect(5, 2, 8, 1, ink);
  }
  if (direction === 'up') {
  ...
  if (variant === 1) {
    rect(direction === 'right' ? 11 : 6, 18, direction === 'right' ? 2 : 6, 1, ink);
  } else if (variant === 2) {
    rect(direction === 'right' ? 11 : 8, 16, 2, 4, ink);
  } else if (variant === 3) {
    rect(direction === 'right' ? 11 : 6, 17, direction === 'right' ? 2 : 2, 2, ink);
    if (direction !== 'right') rect(10, 17, 2, 2, ink);
  } else if (variant === 4) {
    rect(direction === 'right' ? 11 : 7, 17, direction === 'right' ? 2 : 4, 1, shell);
    rect(direction === 'right' ? 11 : 8, 19, 2, 1, ink);
  }
  return data;
}
