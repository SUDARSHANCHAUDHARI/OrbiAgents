...
const prefix = '--verify-isolated-startup=';
...
if (!supplied) {
  ...
  try {
    ...
    if (!isAbsolute(requested)) throw new Error('Verification root must be absolute');
    const root = realpathSync(requested);
    const info = lstatSync(root);
    if (!info.isDirectory() || info.isSymbolicLink()) throw new Error('Verification root must be a real directory');
    if (!readdirSync(root).every((name) => name === '.orbi-isolated-startup'))
      throw new Error('Verification root must contain only its sentinel');
    const sentinel = join(root, '.orbi-isolated-startup');
    if (!lstatSync(sentinel).isFile()) throw new Error('Verification sentinel must be a regular file');
