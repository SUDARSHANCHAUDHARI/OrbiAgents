...
Source license: MIT, copyright Chaitanya Giri, retained in `baseline/LICENSE`. Bundled font notices and SIL OFL text remain beside the fonts. No paid tilesets, maps, upstream branding artwork, GitHub workflows or agent instruction files were imported. The theme registry and OfficeFloor loading path now use the original room, approved LPC props and original robot scene frames. Remaining artwork/branding references elsewhere still require review; do not fill them with paid assets. This integration has not been launched in the full app or visually accepted.
