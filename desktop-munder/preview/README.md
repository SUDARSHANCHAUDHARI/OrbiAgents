...
The preview has loading/error status, resize-to-fit, a Fit room button, attribution links, and disposal of scene textures and image bitmaps. Three original robot designs now use the imported Character controller to walk to desks and take coffee breaks. These are explicitly simulated workers, not live agents. Play/Pause controls the demo; reduced-motion preference starts paused with workers at their desks. The composed room includes licensed cooler, copier and bin props plus original procedural desk monitors; live monitor and errand behavior runs in the migrated application rather than this isolated demo. The migration's normal dev/build/start commands remain blocked.
