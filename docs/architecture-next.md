...
The shared coworking-world model maps real agent states into four purposeful zones: planning, focus, collaboration, and lounge. The web dashboard and VS Code webview consume the same assignments and occupancy summaries. Agents walk between zones as their runtime state changes; active workflow relationships receive a pair-work signal; approval waits, completed work, and mailbox delivery retain distinct visual feedback. Replay frames drive the same mapping, so the visual space follows recorded state rather than a separate simulation.
