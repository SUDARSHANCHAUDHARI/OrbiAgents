...
Accessible quit-warning dialog slice (2026-09-23): the application-closing warning now identifies itself as a named modal alert dialog, exposes graceful-shutdown progress as busy state, takes and contains keyboard focus, and restores the interrupted control when cancelled. Escape maps to cancellation only while cancellation is available; completed shutdown and an in-flight destructive confirmation cannot be interrupted through the shortcut.
