...
| Knowledge relationships | Built (local deterministic) | UI maps durable memories and up to 100 safe local workspace documents, supports ranked bounded snippet queries, and sends selected sourced context to an agent only on explicit operator action; no content leaves the device |
...
| Slack integration | Built, credential activation pending | Operator-supplied `xoxb-` bot token is accepted only from the clipboard, encrypted with OS credential storage, and never exposed to the renderer. Fixed `auth.test` and confirmed `chat.postMessage` calls support channel posts and thread replies using `chat:write` only, with bounded responses and no history scopes, polling, impersonation, or background sends |
