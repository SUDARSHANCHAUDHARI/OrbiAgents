...
| Voice capture and transcription | Built, local runtime activation pending | Explicit start/stop capture uses operator-installed `whisper-cli`, `ffmpeg`, and a selected local GGML model. Commands are fixed argument arrays with bounded input, output, and time; no audio or transcript reaches a cloud service |
