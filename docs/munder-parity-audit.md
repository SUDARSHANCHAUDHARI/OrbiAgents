...
| Knowledge relationships | Built (local deterministic) | UI maps durable memories and up to 100 safe local workspace documents through provenance-backed shared concepts; no content leaves the device |
| Monaco editor and Git history | Built | Safe file broker, save conflict checks, revisions, side-by-side compare, 30-node parent topology, and bounded credential-filtered working-tree patches |
...
| Runtime usage and cost controls | Built with provider limits | Sanitized session signals, elapsed time, bounded provider-reported token/cost facts, and a separate integrity-checked authorization-estimate ledger; missing values are never fabricated |
...
| Inbound webhooks and ephemeral workers | Built (local foundation) | Operator-enabled loopback receiver with per-launch secret, constant-time Bearer verification, replay IDs, bounded payloads, and explicit launch of an isolated 30-minute one-shot worker from a selected local runtime template; receipt alone never executes work |
| Slack integration | Built, credential activation pending | Operator-supplied `xoxb-` bot token is accepted only from the clipboard, encrypted with OS credential storage, and never exposed to the renderer. Fixed `auth.test` and `chat.postMessage` calls use `chat:write` only, bounded responses, explicit confirmation, no history scopes, polling, impersonation, or background sends |
...
The local Apple Silicon macOS coding scope covers the terminal-agent office, twelve built-in runtimes plus custom adapters, coordination, memory and document relationships, IDE and Git/GitHub inspection, skills, provider telemetry, circuit enforcement, setup, recovery, one-shot webhook workers, voice, and controlled updates. Remaining release/acceptance boundaries require external credentials, services, human language review, or platform expansion and must not be presented as completed until those inputs and acceptance tests exist.
