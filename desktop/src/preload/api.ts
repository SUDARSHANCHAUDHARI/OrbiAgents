...
export function createDesktopApi(ipcRenderer: Pick<IpcRenderer, "invoke" | "on" | "removeListener">): OrbiDesktopApi {
  ...
  const hive: OrbiDesktopApi["hive"] = {
    plan: (request) => ipcRenderer.invoke(IPC_CHANNELS.hivePlan, request),
    supervisorStatus: (request) => ipcRenderer.invoke(IPC_CHANNELS.hiveSupervisorStatus, request),
    approvePlan: (request) => ipcRenderer.invoke(IPC_CHANNELS.hiveApprovePlan, request),
    cancelPlan: (request) => ipcRenderer.invoke(IPC_CHANNELS.hiveCancelPlan, request),
    resumePlan: (request) => ipcRenderer.invoke(IPC_CHANNELS.hiveResumePlan, request),
  ...
  const voice: OrbiDesktopApi["voice"] = { policy: () => ipcRenderer.invoke(IPC_CHANNELS.voicePolicy), updatePolicy: (request) => ipcRenderer.invoke(IPC_CHANNELS.voiceUpdatePolicy, request), status: () => ipcRenderer.invoke(IPC_CHANNELS.voiceStatus), chooseModel: () => ipcRenderer.invoke(IPC_CHANNELS.voiceChooseModel), transcribe: (request) => ipcRenderer.invoke(IPC_CHANNELS.voiceTranscribe, request), dispatch: (request) => ipcRenderer.invoke(IPC_CHANNELS.voiceDispatch, request) };
  ...
  const slack: OrbiDesktopApi["slack"] = { status: () => ipcRenderer.invoke(IPC_CHANNELS.slackStatus), saveTokenFromClipboard: () => ipcRenderer.invoke(IPC_CHANNELS.slackSaveToken), saveSigningSecretFromClipboard: () => ipcRenderer.invoke(IPC_CHANNELS.slackSaveSigningSecret), clear: () => ipcRenderer.invoke(IPC_CHANNELS.slackClear), test: () => ipcRenderer.invoke(IPC_CHANNELS.slackTest), send: (request) => ipcRenderer.invoke(IPC_CHANNELS.slackSend, request) };
