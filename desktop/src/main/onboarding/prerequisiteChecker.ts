...
export class PrerequisiteChecker {
  ...
  async check(): Promise<PrerequisiteReport> {
    ...
    const checks: PrerequisiteCheck[] = [
      { id: "platform", label: "Desktop platform", required: true, status: platform === "darwin" || platform === "linux" ? "pass" : "fail", detail: platform === "darwin" ? "Supported macOS desktop platform detected." : platform === "linux" ? "Linux desktop detected. Graphical release acceptance remains pending." : `Current platform ${platform} is not yet supported.` },
      ...
      { id: "mempalace", label: "MemPalace semantic memory", required: false, status: found.mempalace ? "pass" : "warn", detail: found.mempalace ? `mempalace is available at ${found.mempalace} for local semantic recall.` : "Semantic recall is optional; deterministic local text search remains available. Installation requires uv.", ...(!found.mempalace ? { installCommand: "uv tool install mempalace" } : {}) },
