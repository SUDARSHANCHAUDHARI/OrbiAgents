...
export function CommandComposer({ agent, onError }: { agent: AgentSession | null; onError(message: string): void }) {
  ...
  return <section className="command-composer" aria-label={t("commandComposer")}>
    <div className="composer-input"><label htmlFor="agent-command">{t("command")}</label><textarea id="agent-command" aria-label={t("agentCommand")} value={draft} maxLength={8192} disabled={!agent || agent.status !== "running"} placeholder={agent ? `${t("sendInstruction")} ${agent.name}` : t("selectRunningAgent")} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (isCommandQueueShortcut({ key: event.key, metaKey: event.metaKey, ctrlKey: event.ctrlKey, isComposing: event.nativeEvent.isComposing })) { event.preventDefault(); queue(); } }} /><PixelButton type="button" variant="primary" disabled={!draft.trim() || !agent || agent.status !== "running"} onClick={queue}>{t("queue")}</PixelButton></div>
    <div className="composer-files"><select aria-label={t("workspaceAttachment")} value={selectedFile} disabled={!agent || attachments.length >= 5} onChange={(event) => setSelectedFile(event.target.value)}><option value="">{t("attachWorkspaceFile")}</option>{files.filter((file) => !attachments.includes(file.path)).map((file) => <option key={file.path} value={file.path}>{file.path}</option>)}</select><PixelButton type="button" variant="ghost" disabled={!selectedFile || attachments.length >= 5} onClick={() => { setAttachments((current) => [...current, selectedFile]); setSelectedFile(""); }}>{t("attach")}</PixelButton>{attachments.map((file) => <button key={file} type="button" className="composer-file" aria-label={`${t("remove")} ${file}`} onClick={() => setAttachments((current) => current.filter((item) => item !== file))}>{file} ×</button>)}</div>
    <div className="composer-toolbar"><span>{queued.length} {t("queuedEncrypted")}</span><PixelButton type="button" variant="secondary" disabled={!queued.length || !agent || agent.status !== "running" || sending} onClick={() => void sendAll()}>{sending ? t("sending") : t("sendAll")}</PixelButton></div>
    {agentEntries.length ? <ol className="composer-history">{agentEntries.slice(0, 8).map((entry) => <li key={entry.id} data-status={entry.status}><span><strong>{entry.body}</strong><small>{new Date(entry.createdAt).toLocaleTimeString()} · {entry.status}{entry.attachments?.length ? ` · ${entry.attachments.length} ${t("filesCount")}` : ""}</small></span>{entry.status === "queued" || entry.status === "failed" ? <PixelButton type="button" variant="ghost" disabled={!agent || agent.status !== "running" || sending} onClick={() => void send(entry)}>{entry.status === "failed" ? t("retry") : t("send")}</PixelButton> : null}</li>)}</ol> : null}
  </section>;
}
