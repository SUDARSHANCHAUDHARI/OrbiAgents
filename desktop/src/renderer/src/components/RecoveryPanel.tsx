...
export function RecoveryPanel({ onError }: { onError(message: string): void }) {
  ...
  return <PixelPanel title="Startup recovery" eyebrow="read-only inventory" ariaLabel="Startup recovery report" className="recovery-panel" action={<PixelButton type="button" variant="ghost" disabled={busy} onClick={() => void refresh()}>Refresh report</PixelButton>}>
