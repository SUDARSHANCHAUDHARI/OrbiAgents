...
export function ActivityOperationsPanel({ events, agents }: { events: ActivityEvent[]; agents: AgentSession[] }) {
  ...
  return <section className="command-panel activity-operations" aria-label={t("normalizedActivity")}>
    ...
    {visible.length ? <ul>{visible.map((event) => <li key={event.id}><strong>{names.get(event.agentId) ?? event.agentId} · {event.state ?? event.type.replaceAll("-", " ")}</strong><small>{event.source} · {new Date(event.timestamp).toLocaleString()}</small><span>{event.summary}</span></li>)}</ul> : <p className="empty">{t("noSignalMatches")}</p>}
  </section>;
}
